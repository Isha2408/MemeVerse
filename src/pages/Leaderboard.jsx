...
export default function Leaderboard() {
  ...
  useEffect(() => {
    ...
    const userScores = userMemes.reduce((acc, meme) => {
      ...
      acc[user] =
        (acc[user] || 0) +
        parseInt(localStorage.getItem(`meme_likes_${meme.image}`) || "0");
      return acc;
    }, {});
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        {topMemes.length > 0 ? (
          topMemes.map((meme, index) => (
            <motion.div
              key={meme.id}
              className="bg-gray-100 dark:bg-gray-800 p-2 rounded-lg shadow-md flex items-center"
            >
      ...
      {/* Top Users Section */}
      <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-200 mt-8 mb-2">
        👑 Top 5 Users
      </h3>
